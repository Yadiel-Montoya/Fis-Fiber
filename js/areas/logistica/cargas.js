...
async function renderCargas(container) {
  ...
  function render() {
    ...
    setTimeout(() => {
      ...
      CI['g-bar'] = new Chart(bc, {data:{labels:barLabels,datasets:[{type:'bar',label:'Anticipadas',data:data.map(r=>r.Anticipadas),backgroundColor:'rgba(26,58,112,0.85)',borderRadius:3,borderSkipped:false,order:2,datalabels:{display:false}},{type:'bar',label:'Nocturnas',data:data.map(r=>r.Nocturnas),backgroundColor:'rgba(26,158,130,0.85)',borderRadius:3,borderSkipped:false,order:2,datalabels:{display:false}},{type:'line',label:'Total',data:totalesData,borderColor:'#E8892F',backgroundColor:'rgba(232,137,47,0.08)',borderWidth:2.5,pointRadius:4,pointBackgroundColor:'#E8892F',pointBorderColor:'#fff',pointBorderWidth:1.5,fill:false,tension:0.35,order:1,yAxisID:'y',datalabels:{display:false}}]},options:{responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{display:true,position:'bottom',labels:{color:'#5C3038',font:{size:12,family:'Outfit'},usePointStyle:true,pointStyleWidth:14,padding:20}},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${c.parsed.y}`}},datalabels:{display:false}},scales:{x:{grid:{display:false},ticks:{color:tc,font:{size:10,family:mf},maxRotation:45,minRotation:45,autoSkip:true,maxTicksLimit:18},border:{color:'transparent'}},y:{grid:{color:gc},ticks:{color:tc,font:{size:11,family:mf}},border:{color:'transparent'},beginAtZero:true,grace:'15%'}}}});
