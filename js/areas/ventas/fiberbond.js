...
async function renderFiberbond(container) {
    ...
    container.innerHTML = `
      ...
      <div class="filters-bar">
        <div class="filter-group"><span class="filter-label">Mes</span>
          <select class="filter-select" id="fb-mes"><option value="todos">Todos los meses</option>${ALL.map(r => `<option value="${r.mes}"${filtroMes===r.mes?' selected':''}>${r.mes}</option>`).join('')}</select>
        </div>
        <button class="filter-btn" onclick="applyFB()">Aplicar</button>
        <button class="filter-clear" onclick="clearFB()">Limpiar</button>
      ...
      <div class="table-wrap">
        <div class="table-head-bar"><span class="ttl">Detalle mensual · Fiberbond (kg)</span><span class="meta">${D.length} mes${D.length===1?'':'es'}</span></div>
  ...
  window.applyFB = () => { filtroMes = document.getElementById('fb-mes').value; ['fb-genfis','fb-anual'].forEach(DC); render(); };
  window.clearFB = () => { filtroMes = 'todos'; ['fb-genfis','fb-anual'].forEach(DC); render(); };
  render();
}
